...
pub fn route_query(query: &str) -> RetrievalEngine {
    ...
    if query.contains(".rs") || query.contains(".h") || query.contains(".cpp") || query.contains(".c") || query.contains(".py") {
        return RetrievalEngine::FTS;
    }
    // Contains very long words with no spaces (often identifiers or paths)
    if query.split_whitespace().any(|word| word.len() > 14 && !word.contains(|c: char| c.is_ascii_punctuation())) {
        return RetrievalEngine::FTS;
    }
...
pub trait VectorStore {
    ...
    fn search_routed(&self, query: &str, query_embedding: &[f32], limit: usize) -> anyhow::Result<(Vec<SearchResult>, RetrievalEngine)>;
...
pub struct StoreStats { pub documents: usize, pub chunks: usize, pub size_bytes: u64, pub last_ingest: String }
