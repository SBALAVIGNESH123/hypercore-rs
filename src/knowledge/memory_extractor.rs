    ...
    pub fn start_extraction(self) {
        tokio::spawn(async move {
            ...
            let result = task::spawn_blocking(move || {
                extract_memories_from_chunks(&store)
            }).await;

            match result {
                Ok(Ok(count)) => info!("Background Memory Builder: Extracted {} memories from real data.", count),
...
fn get_patterns() -> Vec<MemoryPattern> {
    vec![
        MemoryPattern {
            category: "Decision",
            signals: vec![
                "decided to", "chose ", "switched to", "went with", "picked ",
                "selected ", "opted for", "migrated to", "replaced ", "moved to",
                "we chose", "i chose", "decision to", "agreed to use",
                "settled on", "committed to", "adopted ",
            ],
        },
        MemoryPattern {
            category: "Preference",
            signals: vec![
                "i prefer", "we prefer", "i like ", "prefer ",
                "better than", "rather than", "instead of",
                "strongly prefer", "tend to use", "default to",
                "consistently use", "always use",
            ],
        },
        MemoryPattern {
            category: "Project",
            signals: vec![
                "working on", "building ", "developing ", "project ",
                "started building", "launched ", "shipping ",
                "released ", "deployed ", "prototyping ",
        ...
        MemoryPattern {
            ...
            signals: vec![
                "met with", "collaborated with", "discussed with",
                "worked with", "partnered with", "team includes",
                "reporting to", "hired ", "onboarded ",
                "reviewed with", "pair programmed with",
            ],
        },
    ]
}
...
fn extract_memories_from_chunks(store: &SqliteStore) -> anyhow::Result<usize> {
    ...
    let chunks: Vec<_> = all_chunks.into_iter()
        .filter(|(_path, ext, _content)| {
            NATURAL_LANGUAGE_EXTENSIONS.contains(&ext.as_str())
        })
        .collect();
    ...
    if total_chunks == 0 {
        info!("Background Memory Builder: No natural language chunks found ({} code chunks skipped).", total_all);
        return Ok(0);
    }
    ...
    for (i, (file_path, _ext, content)) in chunks.iter().enumerate() {
        if total_chunks > 20 && (i + 1) % (total_chunks / 5).max(1) == 0 {
            ...
            info!("Building your memory graph... {}% complete ({}/{})", pct, i + 1, total_chunks);
        }
    ...
    info!("Background Memory Builder: Found {} candidate memories. Inserting...", extracted.len());
    ...
    info!("Background Memory Builder: Semantic graph extraction complete! {} real memories stored.", count);
    Ok(count)
}
...
fn is_noise(line: &str) -> bool {
    ...
    if line.contains("::") || line.contains("->") || line.contains("=>")
       ...
       || line.contains(".rs") || line.contains(".py") || line.contains(".js")
       || line.contains("fn ") || line.contains("let ") || line.contains("pub ")
    { return true; }
