...
async fn main() -> anyhow::Result<()> {
    ...
    for file in workload_files {
        ...
        let base_workloads: Vec<WorkloadItem> =
            serde_json::from_str(&workload_data).unwrap_or_default();
        if base_workloads.is_empty() {
            continue;
        }
        ...
        let p_val = compute_p_value(
            b_metrics.avg_ttft_ms,
            b_metrics.variance,
            b_ttfts.len(),
            t_metrics.avg_ttft_ms,
            t_metrics.variance,
            t_ttfts.len(),
        );

        let b_wait_var = compute_variance(
            &b_waits,
            b_waits.iter().sum::<f64>() / b_waits.len().max(1) as f64,
        );
        let t_wait_var = compute_variance(
            &t_waits,
            t_waits.iter().sum::<f64>() / t_waits.len().max(1) as f64,
        );
        ...
        let t_quality = QualityMetrics {
            avg_eviction_count_per_session: t_evictions as f64
                / (workloads.len() * repetitions) as f64,
