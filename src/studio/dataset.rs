...
pub fn generate_dataset() -> anyhow::Result<()> {
    ...
    for row in rows {
        if let Ok((path, content)) = row {
            // Type 1: Retrieval Grounding (Alpaca/ShareGPT format approximation)
            let type1 = json!({
            ...
            writeln!(dataset_file, "{}", type1.to_string())?;
            ...
            writeln!(dataset_file, "{}", type2.to_string())?;
            ...
            writeln!(dataset_file, "{}", type3.to_string())?;

            count += 3;
        }
    }

    info!("Dataset Builder complete! Generated {} high-quality training pairs at hypercore_dataset.jsonl", count);
    Ok(())
}
