...
pub fn export_timeline(_session_id: u64, timeline: &RequestTimeline) {
    ...
    if let (Some(queued), Some(end)) = (queued_at, end_at) {
        let root_builder = SpanBuilder::from_name("inference_request").with_start_time(queued);
        ...
        if let Some(admitted) = admitted_at {
            let wait_builder = SpanBuilder::from_name("request_queue_wait").with_start_time(queued);
        ...
        if let (Some(admitted), Some(first)) = (admitted_at, first_token) {
            let decode_builder = SpanBuilder::from_name("request_decode").with_start_time(admitted);
        ...
        if let (Some(first), Some(last)) = (first_token, end_at) {
            if first < last {
                let stream_builder =
                    SpanBuilder::from_name("request_stream").with_start_time(first);
        ...
        // Cancelled Span
        if let Some(cancelled) = timeline
            .cancelled_at
            .map(|ts| UNIX_EPOCH + Duration::from_millis(ts))
        {
            let cancel_builder =
                SpanBuilder::from_name("request_cancel").with_start_time(cancelled);
