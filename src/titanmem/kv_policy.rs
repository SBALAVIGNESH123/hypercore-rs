...
impl KvCacheObserver for KvCacheController {
    ...
    fn on_kv_extend(&mut self, session_id: u64, batch_tokens: usize, phase: KvPhase) {
        ...
        let additional_bytes =
            (batch_tokens as f64 * self.config.bytes_per_token() as f64 * multiplier) as usize;
    ...
    fn get_schedule(&mut self, active_sessions: &[u64]) -> ScheduleDecision {
        ...
        let target_u_enter =
            (base_enter - a * self.sps_hard_ema + b * self.sps_soft_ema).clamp(0.75, 0.90);
        let target_u_exit =
            (base_exit - a * self.sps_hard_ema + b * self.sps_soft_ema).clamp(0.60, 0.80);
        ...
        if self.current_mode == TitanMemMode::Protective {
            ...
            if utilization < self.current_u_exit
                && growth_rate < growth_threshold
                && active_sessions.len() < self.current_session_limit
            {
            ...
            // Enter Protective
            if utilization > self.current_u_enter
                || growth_rate > growth_threshold
                || active_sessions.len() > self.current_session_limit
            {
